...
function AppContent() {
  ...
  const handleTransactionCreate = async (transactionData: TransactionFormData & { otp: string; skipOTP?: boolean }): Promise<{ transaction: Transaction, otp: string }> => {
    ...
    if (skipOTP) {
      try {
        await applyTransferEffects(newTransaction);
      } catch (error) {
  ...
  const handleTransactionConfirm = async (transactionId: string, inputOTP: string) => {
    ...
    // Apply transfer effects now that OTP is confirmed (inventory + batch allocation)
    try {
      const confirmedTransaction = await transactionService.getTransactionById(transactionId);
      if (confirmedTransaction) {
        await applyTransferEffects(confirmedTransaction);
      }
    } catch (error) {
      logger.warn('Failed to apply transfer effects after OTP confirmation', error);
